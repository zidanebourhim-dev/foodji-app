...
const PHONE_NUMBER = "0537536689"; // Ton Fixe pour les appels

// 📍 COORDONNEES EXACTES (Sala Al Jadida)
// C'est le point central pour calculer les 10km
const RESTO_COORDS = { 
    lat: 33.997484, 
    lng: -6.735644 
}; 
...
// ==========================================
// 2. LISTES DE DONNÉES (MENU & OPTIONS)
// ==========================================
...
const LISTE_SAUCES = [
    "Algérienne Fait Maison", 
    "Biggy Fait Maison", 
    "Barbecue Fait Maison", 
    "Pas de sauce"
]; 
...
function App() {
  ...
  // --- STATES V59 (DISTANCE & BLOCAGE) ---
  const [distanceClient, setDistanceClient] = useState(null);
  const [clientCoords, setClientCoords] = useState(null);
  const [showDistanceBlocker, setShowDistanceBlocker] = useState(false); // C'est le mur des 10km
  ...
  // Connexion Firebase & Écouteurs en temps réel
  useEffect(() => {
  ...
  // Gestion intelligente des catégories (Client vs Admin vs Dimanche)
  ...
  // ==========================================
  // 6. FONCTIONS ADMIN (IMPORT/RESET)
  // ==========================================
  ...
  const ajouterAuPanier = (itemMerged) => {
    ...
    // Ajout d'un ID unique pour chaque item du panier
    const itemSafe = {
        ...itemMerged,
        uniqueId: Date.now()
    };
  ...
  const getPrixItemAjuste = (item) => {
      ...
      // Ajout +5 DH pour la boisson Pep's en livraison/emporter
  ...
  const calculerTotal = () => {
      ...
      // Calcul Promo Dimanche (3ème pizza offerte)
      let remisePromo = 0;
      ...
      // Calcul Frais Livraison (Petite commande)
  ...
  const { sousTotal, remisePromo, fraisLivraison, grandTotal } = calculerTotal();

  // ==========================================
  // 8. LOGIQUE GÉOLOCALISATION V59 (LE MUR)
  // ==========================================
  ...
  const handleOpenPanier = () => {
      ...
      if ("geolocation" in navigator) {
          navigator.geolocation.getCurrentPosition((position) => {
              ...
              // ⛔️ LE MUR DES 10 KM (Priorité Absolue)
              if (dist > 10) {
                  setShowDistanceBlocker(true); // Déclenche l'écran noir
                  return; // Arrête tout ici
              }

              // RÈGLES INTERNES (Si < 10km)
              
              // 1. Gros Panier (>300 DH) : On laisse passer
              if (grandTotal >= 300) {
                  setView('panier');
                  return;
              }
              
              // 2. Zone Lointaine (4-10km) avec Petit Panier : On bloque
              ...
              // Sinon c'est OK (Zone < 4km)
              setView('panier');

          }, (error) => {
              // Si le client refuse le GPS
              if (grandTotal >= 300) {
                  // On laisse passer les gros paniers (doute favorable)
                  setView('panier');
              } else {
          ...
          // Fallback possible ici si besoin
      }
  };
  ...
  const envoyerCommande = async () => {
    ...
    // Sécurité ultime V59 (Au cas où on contourne le blocage visuel)
    if (distanceClient > 10) {
        return alert("Commande impossible : Vous êtes à plus de 10km.");
    }
    ...
    if (grandTotal >= 300) {
        initialStatus = 'En cours de validation'; // Statut Orange
    }
  ...
  const supprimerProduit = async (id) => { 
      if(confirm("Supprimer ?")) await deleteDoc(doc(db, "produits", id)); 
  };
    ...
    <div style={{ background: COLORS.bg, minHeight: '100vh', paddingBottom: '100px', color: COLORS.secondary }}>
      ...
      {view === 'client' && (
        <div style={{ padding: '20px' }}>
          ...
          {/* FLOTTANT PANIER MODIFIÉ V59 (GEO) */}
          {panier.length > 0 && (
