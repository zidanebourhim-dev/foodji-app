...
function App() {
  ...
  // PAR DÉFAUT : FALSE (CACHÉ). Aucune logique ne le passe à TRUE sauf le clic.
  ...
  useEffect(() => {
      ...
      // AUCUNE LOGIQUE ICI POUR AFFICHER LES CGU AUTOMATIQUEMENT
  }, []);
  ...
  useEffect(() => {
    ...
    const unsubscribeCmd = onSnapshot(q, (snap) => {
      ...
      if (list.length > prevCommandesLength.current && user) { audioRef.current.play().catch(e => {}); }
    ...
    const unsubscribeSettings = onSnapshot(doc(db, "settings", "config"), (doc) => {
        if (doc.exists()) {
            const data = doc.data();
            if (data.rushUntil && data.rushUntil > Date.now()) { setRushLevel(data.rushLevel || 'orange'); } 
            else { setRushLevel(null); }
        }
    });
  ...
  const handleEnterApp = () => {
      setIsZooming(true);
      setTimeout(() => { setView('client'); setIsZooming(false); }, 700);
  };
  ...
  const checkIsOpen = () => {
      ...
      if (h < 5) { day = day - 1; if (day === -1) day = 6; }
      if (day >= 1 && day <= 4) return (h >= 12 || h < 1);
      if (day === 5) return (h >= 12 || h < 2);
      if (day === 6 || day === 0) return (h >= 18 || h < 2);
      return false;
  };
  ...
  const envoyerCommande = async () => {
    ...
    const panierFinal = panier.map(item => ({ ...item, prixFinal: getPrixItemAjuste(item) }));
  ...
  const saveProduit = async () => {
    ...
    setEditId(null); setNom(''); setPrixBase(''); setVariantes([]); setLoading(false); alert("Modifié/Ajouté !");
  };
  ...
  const supprimerProduit = async (id) => { if (!checkManagerAuth()) return; if(confirm("Confirmer suppression définitive ?")) await deleteDoc(doc(db, "produits", id)); };
  ...
  const updateProductImage = async (id, file) => { const reader = new FileReader(); reader.readAsDataURL(file); reader.onload=(e)=>{ const img=document.createElement("img"); img.src=e.target.result; img.onload=async()=>{ const c=document.createElement("canvas"); const ctx=c.getContext("2d"); const s=800/img.width; c.width=800; c.height=img.height*s; ctx.drawImage(img,0,0,c.width,c.height); await updateDoc(doc(db, "produits", id), { image: c.toDataURL("image/jpeg", 0.7) }); alert("Image OK"); } } };
    ...
    <div style={{ background: COLORS.bg, minHeight: '100vh', paddingBottom: '100px', color: COLORS.secondary }}>
      ...
      {showDistanceBlocker && (
          <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.95)', zIndex:9999, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'30px', color:'white', textAlign:'center'}}>
              ...
              <h2>Trop loin</h2><p>Limité à 10km.</p>
              <a href={`tel:${PHONE_NUMBER}`} style={{background:'white', color:'black', padding:'20px 40px', borderRadius:'50px', textDecoration:'none', fontWeight:'bold'}}>📞 APPELER</a>
              <button onClick={()=>setShowDistanceBlocker(false)} style={{marginTop:'40px', background:'transparent', border:'1px solid #555', color:'#aaa', padding:'10px'}}>Fermer</button>
          </div>
      )}

      {/* POPUP CGU - S'affiche UNIQUEMENT si showCGU est true (activé par clic) */}
      {showCGU && (
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.85)', zIndex: 10000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', backdropFilter: 'blur(5px)' }}>
              <div style={{ background: 'white', width: '100%', maxWidth: '600px', borderRadius: '20px', padding: '25px', maxHeight: '85vh', display: 'flex', flexDirection: 'column', boxShadow: '0 20px 50px rgba(0,0,0,0.5)' }}>
                  <h2 style={{margin: '0 0 20px 0', textAlign: 'center', color: COLORS.primary}}>CONDITIONS GÉNÉRALES</h2>
                  <div style={{flex: 1, overflowY: 'auto', paddingRight: '10px', fontSize: '0.85rem', lineHeight: '1.6', textAlign: 'justify', borderBottom: '1px solid #eee', marginBottom: '20px'}}>
                      <p><strong>PRÉAMBULE</strong><br/>L'accès et l'utilisation de l'application Foodji impliquent l'acceptation intégrale des présentes Conditions.</p>
                      ...
                      <p><strong>ARTICLE 2 : GÉOLOCALISATION</strong><br/>GPS obligatoire. Aucune livraison au-delà de 10km.</p>
                      <p><strong>ARTICLE 3 : ZONES DE LIVRAISON</strong><br/>- Zone 0-4km : Pas de minimum.<br/>- Zone 4-10km : Minimum 300 DH.</p>
                      <p><strong>ARTICLE 4 : ZONES SPÉCIALES</strong><br/>Supplément 10-15 DH pour UIR, Technopolis, UM6P.</p>
                      <p><strong>ARTICLE 5 : PRODUITS</strong><br/>Photos non contractuelles. Stocks limités.</p>
                      <p><strong>ARTICLE 6 : VALIDATION</strong><br/>Validation téléphonique obligatoire pour commandes > 300 DH.</p>
                      <p><strong>ARTICLE 7 : PAIEMENT</strong><br/>Intégral à la livraison. Tout abus entraînera un bannissement.</p>
                      <p><strong>ARTICLE 8 : HORAIRES</strong><br/>Lun-Jeu: 12h-01h. Ven: 12h-02h. Sam-Dim: 18h-02h.</p>
                      <p><strong>ARTICLE 9 : RUSH</strong><br/>Délais allongés en cas de forte affluence.</p>
                      <p><strong>ARTICLE 10 : ALLERGIES</strong><br/>À signaler impérativement en commentaire.</p>
                      <p><strong>ARTICLE 11 : DROIT</strong><br/>Soumis au droit marocain.</p>
                  </div>
                  <button onClick={()=>setShowCGU(false)} style={{...btnStyle, padding:'15px', fontSize:'1.1rem'}}>FERMER</button>
              </div>
          </div>
      )}

      {rushLevel && !showCGU && view === 'landing' && (
          <div style={{position:'fixed', bottom:'140px', left:'5%', width:'90%', background: rushLevel === 'red' ? '#FEF2F2' : '#FFF7ED', border: rushLevel === 'red' ? '2px solid red' : '2px solid orange', padding:'15px', borderRadius:'15px', zIndex:5000, textAlign:'center', boxShadow:'0 10px 30px rgba(0,0,0,0.2)'}}>
              <h3 style={{margin:'0 0 5px 0', color: rushLevel === 'red' ? '#991B1B' : '#C2410C'}}>{rushLevel === 'red' ? '🚨 EXPLOSION EN CUISINE' : '🔥 FORTE AFFLUENCE'}</h3>
              <p style={{margin:0, fontSize:'0.9rem', color: rushLevel === 'red' ? '#7F1D1D' : '#9A3412'}}>{rushLevel === 'red' ? 'Délais très longs (> 1h). Merci de votre patience.' : 'Les délais de livraison sont allongés (+45min).'}</p>
          </div>
      )}
      ...
      {view === 'landing' && (
        <div style={{ position: 'fixed', top:0, left:0, width: '100%', height: '100%', background: 'linear-gradient(135deg, #1A1E29 0%, #000000 100%)', color: 'white', zIndex: 2000, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflow:'hidden' }}>
          <img src={logoImg} alt="Foodji" className={`logo-idle ${isZooming?'super-zoom':''}`} style={{ width: '250px', height: '250px', objectFit: 'contain', marginBottom: '40px', zIndex: 10 }} onError={(e)=>e.target.style.display='none'} /> 
          
          <div className={isZooming?'fade-out':'content-enter'} style={{textAlign:'center', marginTop:'40px', width:'100%', maxWidth:'300px', padding:'0 20px'}}>
              <button onClick={handleEnterApp} style={{ background: COLORS.primary, color: 'white', border: 'none', padding: '18px 0', width:'100%', borderRadius: '50px', fontSize: '1.2rem', fontWeight: 'bold', boxShadow: '0 10px 30px rgba(168, 68, 56, 0.5)', cursor:'pointer' }}>VOIR LE MENU</button>
              ...
              {derniereCommande && <button onClick={() => setView('ticket')} style={{ marginTop: '30px', display:'block', margin:'30px auto 0 auto', background: 'transparent', border: '1px solid #374151', color: COLORS.primary, padding: '10px 20px', borderRadius: '30px', cursor:'pointer', fontSize:'0.9rem' }}>📄 Ma dernière commande</button>}
          </div>
        </div>
      )}
      ...
      {view !== 'landing' && (
        <div style={{ background: COLORS.card, padding: '15px 20px', position: 'sticky', top: 0, zIndex: 50, display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          <div style={{display:'flex', alignItems:'center', gap:'10px', cursor:'pointer'}} onClick={() => setView('landing')}><img src={iconImg} style={{height:'35px', objectFit:'contain'}} alt="Accueil" /><h1 style={{ margin: 0, fontSize: '1.5rem', fontWeight: '800', letterSpacing: '-0.5px', color: COLORS.secondary }}>Foodji</h1></div>
          {user ? <button onClick={() => setView(view === 'admin' ? 'client' : 'admin')} style={{background: COLORS.secondary, color: 'white', border: 'none', padding: '8px 15px', borderRadius: '20px', fontSize:'0.8rem', fontWeight:'600'}}>{view === 'admin' ? 'App' : 'Admin'}</button> : (view === 'client' && <button onClick={() => setView('login')} style={{background:'transparent', border:'none', fontSize:'1.5rem'}}>🔒</button>)}
        </div>
      )}
      ...
      {view === 'client' && (
        <div style={{ padding: '20px' }}>
          {rushLevel && <div style={{background: rushLevel==='red'?'#FEF2F2':'#FFF7ED', border: rushLevel==='red'?'1px solid #FCA5A5':'1px solid #FDBA74', color: rushLevel==='red'?'#991B1B':'#C2410C', padding:'10px', borderRadius:'10px', marginBottom:'20px', textAlign:'center', fontWeight:'bold'}}>{rushLevel==='red'?'🚨 EXPLOSION (1H+)':'🔥 FORTE AFFLUENCE (45min)'}</div>}
          <div style={{ overflowX: 'auto', whiteSpace: 'nowrap', paddingBottom: '20px', scrollbarWidth: 'none', display:'flex', gap:'10px' }}>
            {categoriesClient.map(c => (<button key={c} onClick={() => setCategorieActive(c)} style={{border: 'none', display:'inline-block', padding:'10px 20px', borderRadius:'25px', background: categorieActive === c ? (c === '🔥 PROMOTIONS' ? COLORS.promo : COLORS.secondary) : 'white', color: categorieActive === c ? 'white' : COLORS.secondary, fontWeight:'600', fontSize:'0.9rem', boxShadow: '0 2px 5px rgba(0,0,0,0.05)', cursor: 'pointer', transition: '0.2s'}}>{c}</button>))}
          </div>
            ...
            {menuClient.map((plat) => {
              ...
              <div key={plat.id} onClick={() => setSelectedProduct(plat)} style={{ ...cardStyle, padding: 0, overflow: 'hidden', display:'flex', flexDirection:'column', cursor: 'pointer', position: 'relative' }}>
                ...
                <div style={{padding:'10px', flex:1, display:'flex', flexDirection:'column', justifyContent:'space-between'}}>
                  <div><h4 style={{ margin: '0 0 5px 0', fontSize: '1rem', fontWeight:'700', color: COLORS.secondary }}>{plat.nom}</h4><p style={{ fontSize: '0.8rem', color: COLORS.textLight, margin: 0, lineHeight:'1.2', display:'-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient:'vertical', overflow:'hidden' }}>{plat.description}</p></div>
                  {!plat.isPromoTrigger && (<div style={{ marginTop: '12px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontWeight: '700', fontSize: '1rem', color: COLORS.primary }}>{displayPrice > 0 ? (plat.variantes?.length > 0 ? `dès ${displayPrice} DH` : `${displayPrice} DH`) : 'GRATUIT'}</span><div style={{background: COLORS.secondary, color: 'white', width: '32px', height: '32px', borderRadius: '50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'1.2rem'}}>+</div></div>)}
                </div>
              </div>
            )})}
          </div>
          {panier.length > 0 && (
            <div onClick={handleOpenPanier} style={{position: 'fixed', bottom: '30px', left: '5%', width: '90%', background: COLORS.secondary, color: 'white', padding: '15px 25px', borderRadius: '50px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxShadow: '0 10px 25px rgba(0,0,0,0.2)', cursor: 'pointer', zIndex: 99}}>
              <div style={{display:'flex', alignItems:'center', gap:'12px'}}><span style={{background: COLORS.primary, color:'white', width:'28px', height:'28px', borderRadius:'50%', display:'flex', justifyContent:'center', alignItems:'center', fontWeight:'bold', fontSize:'0.9rem'}}>{panier.length}</span><span style={{fontSize:'1rem', fontWeight:'500'}}>Voir le panier</span></div><span style={{fontWeight:'800', fontSize:'1.1rem'}}>{grandTotal} DH</span>
            </div>
          )}
        </div>
      )}
      ...
      {view === 'panier' && (
        <div style={{ padding: '20px', background: 'white', minHeight: '100vh' }}>
          ...
          {typeCommande === 'livraison' && <div style={{background: '#FEF2F2', border: '1px solid #FCA5A5', padding:'10px', borderRadius:'8px', marginBottom:'20px', fontSize:'0.9rem', color: '#B91C1C'}}><strong>⚠️ Info Zones Spéciales :</strong><br/>Pour <strong>UIR, Technopolis, UM6P</strong>, un supplément (10-15 DH) sera demandé par le livreur.</div>}
          {panier.length === 0 ? <p>Panier vide.</p> : (
            <>
              <div style={{marginBottom:'30px'}}>
                {panier.map(item => (
                  <div key={item.uniqueId} style={{display:'flex', justifyContent:'space-between', padding:'15px 0', borderBottom:'1px solid #f0f0f0'}}>
                    <div><div style={{fontSize:'0.75rem', fontWeight:'bold', color: COLORS.primary, marginBottom:'2px'}}>{item.categorie.toUpperCase()}</div><div style={{fontWeight:'600'}}>{item.nom} {item.choixPates && <span style={{color: COLORS.secondary, fontWeight:'bold'}}> ({item.choixPates})</span>} {item.varianteNom && <span style={{color: COLORS.textLight}}> ({item.varianteNom})</span>}</div><div style={{color: COLORS.textLight, fontSize:'0.9rem'}}>{item.isCheesyCrust && <div style={{color: COLORS.promo, fontWeight:'bold'}}>+ Cheesy Crust</div>}{item.extras && item.extras.length > 0 && <div>+ {item.extras.map(e => e.nom).join(', ')}</div>}{item.sauces && item.sauces.length > 0 && <div>Sauces: {formatOptions(item.sauces)}</div>}{item.optionsChoisies && item.optionsChoisies.length > 0 && <div>+ {formatOptions(item.optionsChoisies)}</div>}{item.sans && item.sans.length > 0 && <div style={{color: COLORS.danger}}>Sans: {item.sans.join(', ')}</div>}</div></div>
                    <div style={{display:'flex', gap:'15px', alignItems:'center'}}><strong style={{color: COLORS.primary}}>{getPrixItemAjuste(item)} DH</strong><button onClick={() => retirerDuPanier(item.uniqueId)} style={{color:'#ccc', background:'transparent', border:'none', fontSize:'1.5rem'}}>×</button></div>
                  </div>
                ))}
                {remisePromo > 0 && <div style={{background: '#ECFDF5', color: COLORS.success, padding:'10px', borderRadius:'8px', marginTop:'15px', fontWeight:'bold', textAlign:'center'}}>🎁 Promo Dimanche : -{remisePromo} DH</div>}
                {fraisLivraison > 0 && typeCommande === 'livraison' && <div style={{textAlign:'right', color: COLORS.textLight, marginTop:'10px'}}>+ Frais livraison : 5 DH</div>}
              ...
              <div style={{background: COLORS.bg, padding: '20px', borderRadius: '16px'}}>
                ...
                <div style={{display:'flex', gap:'10px', marginBottom:'15px'}}>{['sur_place', 'emporter', 'livraison'].map(t => (<button key={t} onClick={() => setTypeCommande(t)} style={{flex:1, padding:'10px 5px', borderRadius:'10px', border: typeCommande===t ? `2px solid ${COLORS.secondary}` : '1px solid #ddd', background: typeCommande===t ? COLORS.secondary : 'white', color: typeCommande===t ? 'white' : COLORS.textLight, fontWeight:'600', fontSize:'0.85rem'}}>{t.replace('_',' ')}</button>))}</div>
                ...
                <textarea value={commentaire} onChange={e => setCommentaire(e.target.value)} style={{...inputStyle, height:'60px', marginTop:'10px'}} placeholder="Commentaire..." />
      ...
      {view === 'ticket' && derniereCommande && (
          <div style={{padding: '20px', background: COLORS.bg, minHeight: '100vh', display:'flex', flexDirection:'column', alignItems:'center'}}>
              <div style={{background: 'white', padding: '30px 20px', borderRadius: '20px', boxShadow: '0 10px 25px rgba(0,0,0,0.1)', width: '100%', maxWidth: '400px', textAlign: 'center'}}>
                  <div style={{width:'60px', height:'60px', background: derniereCommande.status === 'En cours de validation' ? COLORS.pending : COLORS.success, color:'white', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'2rem', margin:'0 auto 20px auto'}}>{derniereCommande.status === 'En cours de validation' ? '!' : '✓'}</div>
                  {derniereCommande.status === 'En cours de validation' ? <><h2 style={{margin: '0 0 10px 0', color: COLORS.pending}}>En attente de validation</h2><p style={{color: COLORS.textLight, fontSize:'0.9rem', marginBottom:'30px'}}>Votre commande dépasse 300 DH. Nous allons vous appeler pour la valider.</p></> : <><h2 style={{margin: '0 0 10px 0', color: COLORS.secondary}}>Commande Transmise !</h2><p style={{color: COLORS.textLight, fontSize:'0.9rem', marginBottom:'30px'}}>Votre commande a bien été reçue.</p></>}
                  <div style={{borderTop: '2px dashed #eee', borderBottom: '2px dashed #eee', padding: '20px 0', marginBottom: '20px', textAlign:'left'}}><div style={{display:'flex', justifyContent:'space-between', marginBottom:'10px', fontWeight:'bold'}}><span>Commande N°</span><span>#{derniereCommande.id ? derniereCommande.id.slice(-4).toUpperCase() : '----'}</span></div><ul style={{listStyle:'none', padding:0, marginTop:'20px'}}>{derniereCommande.items.map((it, i) => (<li key={i} style={{marginBottom:'10px', fontSize:'0.95rem', borderBottom:'1px solid #f9f9f9', paddingBottom:'5px'}}><div style={{display:'flex', justifyContent:'space-between'}}><span>{it.nom} {it.varianteNom && `(${it.varianteNom})`}</span><span style={{fontWeight:'bold'}}>{it.prixFinal} DH</span></div></li>))}</ul></div>
                  <div style={{display:'flex', justifyContent:'space-between', fontSize:'1.2rem', fontWeight:'800', color: COLORS.primary}}><span>TOTAL</span><span>{derniereCommande.total} DH</span></div>
              </div>
              <button onClick={() => { setView('client'); }} style={{marginTop:'30px', background: COLORS.secondary, color:'white', border:'none', padding:'15px 30px', borderRadius:'30px', fontWeight:'bold', cursor:'pointer'}}>Commander à nouveau</button>
          </div>
      )}
      ...
      {view === 'admin' && user && (
        <div style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
          <div style={{marginBottom:'20px', display:'flex', gap:'10px', alignItems:'center', justifyContent:'space-between'}}>
              <div style={{display:'flex', alignItems:'center', gap:'10px'}}><h2 style={{margin:0}}>⚙️ Admin</h2><div style={{fontSize:'0.8rem', color: COLORS.success, background:'#ECFDF5', padding:'5px 10px', borderRadius:'10px'}}>🔊 Son Actif</div></div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '20px', marginBottom:'40px' }}>
            {commandes.map(cmd => (
              <div key={cmd.id} style={{ ...cardStyle, borderLeft: cmd.status === 'Terminé' ? '5px solid #ccc' : (cmd.status === 'En cours de validation' ? `5px solid ${COLORS.pending}` : `5px solid ${COLORS.success}`) }}>
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'start', marginBottom:'15px', paddingBottom:'15px', borderBottom:'1px solid #f0f0f0'}}>
                  <div><strong style={{fontSize:'1.2rem', display:'block'}}>{cmd.client}</strong><div style={{color: COLORS.textLight, marginTop:'4px'}}>📞 {cmd.tel}</div><div style={{marginTop:'5px', fontSize:'0.8rem', fontWeight:'bold', color: COLORS.secondary, display:'flex', gap:'10px', alignItems:'center'}}><span>📍 {cmd.distance} km</span>{cmd.lat && cmd.lng && (<a href={`https://www.google.com/maps/search/?api=1&query=${cmd.lat},${cmd.lng}`} target="_blank" rel="noreferrer" style={{color: COLORS.primary, textDecoration:'underline'}}>Voir Map</a>)}</div></div>
                  <div style={{textAlign:'right'}}><div style={{fontSize:'1.3rem', fontWeight:'bold', color: COLORS.primary}}>{cmd.total} DH</div><button onClick={() => copierOdoo(cmd)} style={{marginTop:'5px', background: COLORS.secondary, color:'white', border:'none', padding:'6px 12px', borderRadius:'6px', fontSize:'0.75rem', cursor:'pointer'}}>📋 COPIER</button></div>
                </div>
                ...
                <ul style={{listStyle:'none', marginBottom:'15px'}}>{cmd.items && cmd.items.map((it, i) => (<li key={i} style={{padding:'8px 0', borderBottom:'1px dashed #eee', lineHeight:'1.4'}}><div style={{display:'flex', alignItems:'center', gap:'10px', marginBottom:'2px'}}><span style={{fontSize:'0.75rem', fontWeight:'bold', color: COLORS.primary, background:'#FEE2E2', padding:'2px 6px', borderRadius:'4px'}}>[{it.categorie ? it.categorie.toUpperCase() : 'PLAT'}]</span><strong style={{fontSize:'1.1rem'}}>{it.nom}</strong></div><div style={{display:'flex', justifyContent:'space-between', color: COLORS.secondary}}><span>{it.choixPates && <strong style={{color: COLORS.primary, marginRight:'5px'}}>{it.choixPates}</strong>} {it.varianteNom && <strong style={{color: COLORS.secondary, fontSize:'0.95rem'}}>({it.varianteNom})</strong>}</span><strong style={{color: COLORS.textLight}}>{it.prixFinal} DH</strong></div><div style={{fontSize:'0.85rem', color:'#444', marginLeft:'10px', marginTop:'2px'}}>{it.isCheesyCrust && <div style={{fontWeight:'bold', color: COLORS.promo}}>★ CHEESY CRUST</div>}{it.extras && it.extras.length > 0 && <div>+ {it.extras.map(e => e.nom).join(', ')}</div>}{it.sauces && it.sauces.length > 0 && <div>Sauces: {formatOptions(it.sauces)}</div>}{it.optionsChoisies && it.optionsChoisies.length > 0 && <div>+ {formatOptions(it.optionsChoisies)}</div>}{it.sans && it.sans.length > 0 && <div style={{color: COLORS.danger, fontWeight:'bold'}}>🚫 {it.sans.join(', ')}</div>}</div></li>))}</ul>
                <div style={{display:'flex', gap:'10px', flexWrap:'wrap'}}>
                  {cmd.status === 'En cours de validation' ? (<><button onClick={()=>changerStatus(cmd.id, 'En attente')} style={{...btnStyle, background: COLORS.success, padding:'10px', flex:1}}>☎️ CLIENT OK</button><button onClick={()=>changerStatus(cmd.id, 'Refusé')} style={{...btnStyle, background: COLORS.danger, padding:'10px', flex:1}}>REFUSER</button></>) : (<>{cmd.status !== 'Terminé' && cmd.status !== 'Refusé' && <button onClick={()=>changerStatus(cmd.id, 'Terminé')} style={{...btnStyle, background: COLORS.success, padding:'10px'}}>✅ SERVI</button>}<button onClick={()=>supprimerCmd(cmd.id)} style={{...btnStyle, background:'white', color:'red', border:'1px solid #eee', padding:'10px'}}>🗑️</button></>)}
                </div>
              </div>
            ))}
          </div>
           <div style={{marginTop:'40px', borderTop:'2px solid #eee', paddingTop:'20px'}}>
             ...
             <div style={{ overflowX: 'auto', whiteSpace: 'nowrap', paddingBottom: '15px', display:'flex', gap:'10px' }}>
              {categoriesReelles.map(c => (<button key={c} onClick={() => setAdminCategorie(c)} style={{padding:'8px 15px', borderRadius:'20px', border:'none', background: adminCategorie===c?COLORS.secondary:'#eee', color:adminCategorie===c?'white':'black', cursor:'pointer'}}>{c}</button>))}
             ...
             {menuAdmin.map(p => (
              <div key={p.id} style={{display:'flex', justifyContent:'space-between', alignItems:'center', padding:'12px', borderBottom:'1px solid #f0f0f0', background: p.available === false ? '#FFF5F5' : 'white', opacity: p.available === false ? 0.7 : 1}}>
                <div style={{display:'flex', alignItems:'center', gap:'15px'}}>
                  <div onClick={() => toggleAvailability(p)} style={{width:'50px', height:'26px', background: p.available !== false ? COLORS.success : '#ccc', borderRadius:'20px', position:'relative', cursor:'pointer', transition:'0.3s'}}><div style={{width:'20px', height:'20px', background:'white', borderRadius:'50%', position:'absolute', top:'3px', left: p.available !== false ? '27px' : '3px', transition:'0.3s'}}></div></div>
                  <div style={{width:'40px', height:'40px', background:'#eee', borderRadius:'5px', overflow:'hidden', position:'relative'}}><img src={p.image} style={{width:'100%', height:'100%', objectFit:'cover'}} /><input type="file" onChange={(e)=>updateProductImage(p.id, e.target.files[0])} style={{position:'absolute', top:0, left:0, width:'100%', height:'100%', opacity:0, cursor:'pointer'}} /></div>
                  <div><div style={{fontWeight:'bold', textDecoration: p.available === false ? 'line-through' : 'none'}}>{p.nom}</div><div style={{fontSize:'0.8rem', color: COLORS.textLight}}>{p.categorie} • {p.variantes?.length > 0 ? 'Multi-tailles' : p.prix + ' DH'}</div></div>
                </div>
                <div style={{display:'flex', gap:'10px'}}><button onClick={() => handleEdit(p)} style={{border:'none', background:'transparent', fontSize:'1.2rem', cursor:'pointer'}}>✏️</button><button onClick={()=>supprimerProduit(p.id)} style={{color:'red', border:'none', background:'transparent', cursor:'pointer'}}>X</button></div>
              </div>
            ))}
          </div>
           <details style={{marginTop:'30px', background:'white', padding:'15px', borderRadius:'10px'}}>
             ...
             <div style={{marginTop:'10px'}}>
                 ...
                 <div style={{display:'flex', gap:'10px', alignItems:'start'}}>
                   <select value={categorie} onChange={handleCategoryChange} style={{...inputStyle, width:'50%'}}>{categoriesSelectAdmin.map(cat => <option key={cat}>{cat}</option>)}</select>
                   {variantes.length > 0 ? (<div style={{width:'50%', display:'flex', gap:'5px', flexWrap:'wrap'}}>{variantes.map((v, index) => (<div key={index} style={{flex:1, minWidth:'80px'}}><label style={{fontSize:'0.7rem', fontWeight:'bold', color: COLORS.textLight}}>{v.nom}</label><input type="number" value={v.prix} onChange={(e) => updateVariantPrice(index, e.target.value)} style={{...inputStyle, marginBottom:0}} /></div>))}</div>) : (<input type="number" placeholder="Prix" value={prixBase} onChange={e=>setPrixBase(e.target.value)} style={{...inputStyle, width:'50%'}} />)}
                 </div>
           ...
           <details style={{marginTop:'50px', background:'#FEE2E2', padding:'15px', borderRadius:'10px', border:`1px solid ${COLORS.danger}`}}>
             ...
             <div style={{marginTop:'20px', display:'flex', gap:'10px', flexDirection:'column'}}><button onClick={triggerImport} style={{...btnStyle, background: 'white', color: COLORS.secondary, border:'1px solid #ccc'}}>📂 IMPORTER UN MENU (CSV)</button><button onClick={viderMenu} style={{...btnStyle, background: COLORS.danger, color:'white'}}>🗑️ TOUT SUPPRIMER (RESET)</button></div>
           </details>
        </div>
      )}
    </div>
...
function PromoWizard({ menu, onClose, onValidate }) {
    ...
    useEffect(() => { setChoix([]); }, []);
    const pizzasEligibles = menu.filter(p => p.categorie === 'Pizzas' && p.available !== false && !PIZZAS_EXCLUES_PROMO.some(ex => p.nom.toLowerCase().includes(ex)));
    const handleSelect = (pizza) => {
        ...
        setChoix([...choix, { ...pizza, prixFinal: Number(prixFinal), originalPrice: Number(prixFinal), varianteNom: varianteNom, isPromoEligible: true }]);
    };
    const handleRemoveChoice = (indexToRemove) => { setChoix(choix.filter((_, index) => index !== indexToRemove)); };
    return (
        <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.9)', zIndex:2000, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'20px'}}>
            <div style={{background:'white', width:'100%', maxWidth:'600px', borderRadius:'20px', padding:'20px', maxHeight:'90vh', overflowY:'auto'}}>
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px'}}><h3 style={{margin:0}}>Choix {choix.length} / 3</h3><button onClick={onClose} style={{border:'none', background:'transparent', fontSize:'1.5rem'}}>×</button></div>
                <div style={{display:'flex', gap:'10px', marginBottom:'20px', background:'#F3F4F6', padding:'10px', borderRadius:'10px'}}>
                    {[0, 1, 2].map(i => (<div key={i} style={{flex:1, height:'60px', background:'white', border:'2px dashed #ddd', borderRadius:'8px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.8rem', textAlign:'center', position:'relative', fontWeight:'bold'}}>{choix[i] ? (<>{choix[i].nom}<div onClick={() => handleRemoveChoice(i)} style={{position:'absolute', top:'-5px', right:'-5px', background:'red', color:'white', width:'20px', height:'20px', borderRadius:'50%', cursor:'pointer', fontSize:'0.7rem', display:'flex', alignItems:'center', justifyContent:'center'}}>×</div></>) : <span style={{color:'#ccc'}}>Vide</span>}</div>))}
                </div>
                {choix.length < 3 ? (<div style={{display:'grid', gridTemplateColumns:'1fr', gap:'10px'}}>{pizzasEligibles.map(p => (<button key={p.id} onClick={() => handleSelect(p)} style={{padding:'15px', borderRadius:'12px', border:'1px solid #eee', background:'white', textAlign:'left', display:'flex', justifyContent:'space-between', alignItems:'center', boxShadow:'0 2px 5px rgba(0,0,0,0.05)', cursor:'pointer'}}><span style={{fontWeight:'bold'}}>{p.nom}</span><span style={{color: COLORS.primary, fontWeight:'bold', background:'#FEE2E2', padding:'5px 10px', borderRadius:'15px'}}>+ Ajouter</span></button>))}</div>) : (<button onClick={() => onValidate(choix)} style={{background: COLORS.success, color:'white', width:'100%', padding:'20px', border:'none', borderRadius:'15px', fontSize:'1.2rem', fontWeight:'bold', cursor:'pointer'}}>✅ VALIDER CES 3 PIZZAS</button>)}
            </div>
        </div>
    );
}
...
function ProductModal({ product, onClose, onAdd }) {
  ...
  if (isMixte) { listeOptions = LISTE_VIANDES; titreOptions = "Choisissez vos viandes"; minChoix = 2; if (selectedVar?.nom === 'L' || selectedVar?.nom === 'Standard') maxChoix = 2; else if (selectedVar?.nom === 'XL') maxChoix = 3; else if (selectedVar?.nom === 'XXL') maxChoix = 4; else maxChoix = 2; }
  else if (isPizza) { if (nomLower.includes('2 saisons')) { maxChoix = 2; minChoix = 2; listeOptions = LISTE_GARNITURES_PIZZA; titreOptions = "2 Garnitures"; } if (nomLower.includes('4 saisons')) { maxChoix = 4; minChoix = 4; listeOptions = LISTE_GARNITURES_PIZZA; titreOptions = "4 Garnitures"; } }
  ...
  return (
    <div style={{position:'fixed', top:0, left:0, width:'100%', height:'100%', background:'rgba(0,0,0,0.5)', zIndex:1000, display:'flex', alignItems:'flex-end', justifyContent:'center'}}>
      <div style={{background:'white', width:'100%', maxWidth:'600px', borderRadius:'20px 20px 0 0', padding:'25px', maxHeight:'90vh', overflowY:'auto'}}>
        <div style={{display:'flex', justifyContent:'space-between', alignItems:'start'}}><h2 style={{margin:0, fontSize:'1.4rem'}}>{product.nom}</h2><button onClick={onClose} style={{border:'none', background:'transparent', fontSize:'1.5rem', fontWeight:'bold'}}>×</button></div>
